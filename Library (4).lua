...
function Library:GetIcon(IconName: string)
    ...
    IconCache[IconName] = Icon or false
    return Icon
end
...
local TextBoundsCacheCount = 0
function Library:GetTextBounds(Text: string, Font: Font, Size: number, Width: number?): (number, number)
    local CacheKey = table.concat({
        #Text, Text, Width or workspace.CurrentCamera.ViewportSize.X - 32,
        Size, Font.Face, Font.Weight.Value, Font.Style.Value,
    }, "\0")
    ...
    if TextBoundsCacheCount >= 1024 then
        TextBoundsCache = {}
        TextBoundsCacheCount = 0
    end
    TextBoundsCache[CacheKey] = { Bounds.X, Bounds.Y }
    TextBoundsCacheCount += 1
...
function Library:SetGradientAnimation(State: boolean)
    ...
    if State then
        if not Library.GradientConnection then
            local t = 0
            local GradAccent, GradMain = nil, nil
            local LastBreathe = 0
            Library.GradientConnection = RunService.Heartbeat:Connect(function(dt)
                ...
                if Library.GradientColor then
                    ...
                    local Accent, Main = Library.Scheme.AccentColor, Library.Scheme.MainColor
                    if Accent ~= GradAccent or Main ~= GradMain then
                        GradAccent, GradMain = Accent, Main
                        Library.GradientColor.Color = ColorSequence.new({
                            ColorSequenceKeypoint.new(0, Accent),
                            ColorSequenceKeypoint.new(0.5, Main),
                            ColorSequenceKeypoint.new(1, Accent),
                        })
                    end
                    local Now = tick()
                    if Now - LastBreathe >= 0.05 then
                        LastBreathe = Now
...
function Library:CreateWindow(WindowInfo)
    ...
    -- Gotham is forced for the whole UI
    WindowInfo.Font = Font.fromEnum(Enum.Font.Gotham)
    WindowInfo.CornerRadius = math.clamp(WindowInfo.CornerRadius, 16, 24)
    ...
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        SearchDebounceThread = task.delay(0.15, function()
            ...
            if not Library.Unloaded and SearchBox.Text == Text then
                Library:UpdateSearch(Text)
            end
        end)
    end)
    Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
...
do
    ...
    function Library:SetupWatermark(config)
        ...
        _WMLabel.Font = Enum.Font.Gotham
        ...
        local _WMStatsNetwork = nil
        local _WMPingItem = nil
        local _WMLastAccent, _WMLastMain, _WMLastBg, _WMLastFontColor = nil, nil, nil, nil
        _WMFPSConn = RunService.RenderStepped:Connect(function()
            _WMFrameCount += 1
        end)
        _WMConn = RunService.Heartbeat:Connect(function()
            local Config = Library.WatermarkConfig
            if not Config.ShowWatermark then
                if _WMFrame.Visible then
                    _WMFrame.Visible = false
                end
                return
            end
            local Now = tick()
            if Now - _WMLastUpdate < 0.25 then
                return
            end
            local Elapsed = Now - _WMLastTime
            if Elapsed >= 0.5 then
                _WMFPS = math.floor(_WMFrameCount / Elapsed)
                _WMFrameCount = 0
                _WMLastTime = Now
            end
            _WMLastUpdate = Now
            if not _WMFrame.Visible then
            ...
            local FontCol = Library.Scheme.FontColor
            if Accent ~= _WMLastAccent or Main ~= _WMLastMain or Bg ~= _WMLastBg or FontCol ~= _WMLastFontColor then
                _WMLastAccent, _WMLastMain, _WMLastBg, _WMLastFontColor = Accent, Main, Bg, FontCol
                _WMFrame.BackgroundColor3 = Bg
                _wmStroke.Color = Accent
                _wmGradient.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Accent),
                    ColorSequenceKeypoint.new(0.15, Main),
                    ColorSequenceKeypoint.new(1, Bg),
                })
                _WMLabel.TextColor3 = FontCol
            end
            local Parts = { Config.ScriptName }
            ...
            if Config.ShowPing then
                ...
                local Ok, Result = pcall(function()
                    if not _WMPingItem or not _WMPingItem.Parent then
                        _WMStatsNetwork = _WMStatsNetwork or game:GetService("Stats"):FindFirstChild("Network")
                        if _WMStatsNetwork then
                            _WMPingItem = _WMStatsNetwork:FindFirstChild("Data Ping")
                        end
                    end
                    return _WMPingItem and math.floor(_WMPingItem.Value) or nil
                end)
            ...
            if _WMLabel.Text ~= NewText then
                ...
                local Size = TextService:GetTextSize(NewText, 13, Enum.Font.Gotham, Vector2.new(math.huge, math.huge))
